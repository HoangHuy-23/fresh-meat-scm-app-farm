...
export default function Shipments() {
  ...
  const fetchPickingUpShipments = useCallback(async () => {
    // IMPORTANT: Replace with the actual ID of the current user's farm
    const MY_FARM_ID = "farm-a";

    try {
      const response = await shipmentApi.getMyPickupShipments(MY_FARM_ID);

      const processedData = response
        .map((shipment: any): ProcessedShipment | null => {
          // Find the specific stop at our farm where a pickup is scheduled
          const pickupStop = shipment.stops.find(
            (stop: any) =>
              stop.facilityID === MY_FARM_ID && stop.action === "PICKUP"
          );
